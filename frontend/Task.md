...
# AVTO (YHQ) — Dizayner uchun To'liq Prompt

Bu safar kod emas, balki **dizaynerga tushunarli, vizual tilda** yozilgan brifing kerak. Quyida shuni tayyorladim — har bir sahifa, har bir animatsiya, va "vov effekti" beradigan detallar bilan.

```
════════════════════════════════════════════════════════════════
DIZAYN BRIFI: AVTO (YHQ) — Haydovchilik Ta'lim Platformasi
════════════════════════════════════════════════════════════════

── LOYIHA MOHIYATI (dizayner tushunishi uchun) ────────────────────

Bu — YHQ (Yo'l Harakati Qoidalari) o'rgatuvchi ta'lim platformasi.
ASOSIY XUSUSIYAT: foydalanuvchi savolga javob berganda, ekranda
JONLI 2D ANIMATSIYA ko'rsatiladi — mashinalar chorrahada harakatlanadi.
Agar javob to'g'ri bo'lsa — mashinalar muammosiz o'tib ketadi.
Agar xato bo'lsa — mashinalar TO'QNASHADI yoki qoida buzilishi
vizual ko'rsatiladi (masalan, boshqa mashina keskin tormoz beradi).

Bu — video EMAS, real vaqtda chiziladigan animatsiya (o'yin dvigateli
kabi). Shuning uchun dizayn "o'yin+ta'lim" (edutainment) hissini
berishi kerak — jiddiy imtihon tayyorgarligi, lekin zerikarli emas.

── UMUMIY VIZUAL YO'NALISH ────────────────────────────────────────

Uslub:      Zamonaviy, "tungi shahar" mavzusi (dark mode, neon-vibe)
Hissiyot:   Ishonchli + texnologik + biroz adrenalin (imtihon kutish
            hissi kabi, lekin qo'rqinchli emas — professional)
Solishtirma: Duolingo'ning o'yinchan tuzilishi + Tesla ilovasining
            texnologik minimalizmi + simulyator o'yinlarining HUD dizayni

RANG PALITRASI (mavjud, saqlanadi va rivojlantiriladi):
  Fon:        #0b0f19 (eng qorong'i) → #151c2c → #1e293b (gradient qatlamlar)
  Ko'k accent:   #3b82f6  — asosiy tugmalar, linklar
  Moviy-yashil:  #06b6d4  — ikkinchi darajali accent, hover holatlar
  Yashil:        #10b981  — TO'G'RI JAVOB, muvaffaqiyat
  Qizil:         #ef4444  — XATO JAVOB, to'qnashuv, xavf
  Amber:         #f59e0b  — ogohlantirish, imtiyoz buzilishi
  Binafsha:      #8b5cf6  — premium/maxsus elementlar (masalan Exam rejimi)

TIPOGRAFIYA:
  Sarlavhalar:  Outfit (bold, zamonaviy geometrik)
  Matn:         Plus Jakarta Sans (o'qilishi oson, iliq)

EFFEKTLAR (loyihaning "imzosi"):
  - Glassmorphism: shaffof, xira fon (backdrop-blur) — kartochkalar,
    navbar, modal oynalarda
  - Glow soyalar: tugmalar hover qilinganda rangli nurlanish
    (masalan yashil tugma atrofida yumshoq yashil soya)
  - Gradient matn: sarlavhalarda ko'k→moviy-yashil gradient

── SAHIFALAR RO'YXATI VA HAR BIRI UCHUN DIZAYN ────────────────────

1) BOSH SAHIFA (Home)
   Maqsad: birinchi taassurot — "bu zamonaviy va ishonchli platforma"
   Tarkib:
   - Hero bo'lim: katta sarlavha + subtitle + "Boshlash" tugmasi,
     orqa fonda xira, sekin harakatlanuvchi chorraha animatsiyasi
     (parallax effekt bilan, foydalanuvchi diqqatini chalg'itmasdan)
   - 4 ta statistika kartochka (masalan "20+ ssenariy", "13 mavzu")
     — sahifa scroll qilinganda raqamlar 0'dan sanab ko'tarilib boradi
   - 5 ta feature kartochka (nima uchun bu platforma) — hover
     qilinganda kartochka biroz katta bo'lib, glow chiqadi

2) DARSLIKLAR (Lessons)
   Layout: chap tomonda mavzular ro'yxati (sidebar), o'ng tomonda
   tanlangan mavzu tafsiloti
   Muhim detal: har bir mavzu yonida kichik belgi ikonkasi
   (masalan svetofor, imtiyoz belgisi) — vizual tez taniladigan

3) MASHQ (Practice) — ⭐ ENG MUHIM SAHIFA
   Bu yerda asosiy "vov effekti" bo'lishi kerak. Tuzilishi:
   - Yuqorida: mavzu filtri (pills/chips shaklida tugmalar)
   - O'rtada: katta Canvas animatsiya maydoni (chorraha, mashinalar)
   - Animatsiya ostida: savol matni + 2-5 ta javob variant tugmasi
   - Javob tanlanganda: TUGMALAR VAQTINCHA O'CHADI (disabled holat),
     Canvas'da animatsiya davom etadi — mashina harakatlanadi
   - Natija: agar to'g'ri — yashil ✅ banner yumshoq slide-in bilan
     pastdan chiqadi + engil "ding" ovoz effekti (ixtiyoriy)
   - Agar xato — qizil 💥 banner, LEKIN ekran qizarib
     ketmasligi kerak (bu tarbiyaviy ilova, qo'rqitish emas) —
     shunchaki aniq, jiddiy xabar: "Xato: ... sababli to'qnashuv"
   - Pastda: Oldingi/Keyingi ssenariy tugmalari

4) IMTIHON (Exam)
   Farqi Practice'dan: yuqorida katta TAYMER (20:00 dan orqaga sanaydi)
   - Taymer oxirgi 2 daqiqada RANGI qizil rangga o'tishi kerak
     (foydalanuvchini shoshiltirish uchun, lekin bezovta qilmasdan)
   - Pastda savol-navigatsiya panel: 20 ta kichik kvadrat,
     javob berilganlari yashil, berilmaganlari kulrang
   - Natija sahifasi: katta doira grafik (masalan 18/20),
     "O'tdingiz!" yoki "Qayta urinib ko'ring" xabari bilan

5) STATISTIKA (Analytics)
   - Yuqorida 3-4 ta summary kartochka (jami urinish, to'g'ri %)
   - Pastda jadval: oxirgi 10 ta urinish, har birida sana,
     mavzu, natija (✅/❌ ikonka bilan)
   - Ixtiyoriy: progress vaqt bo'yicha chiziqli grafik

6) KIRISH / RO'YXATDAN O'TISH (Login/Register)
   - Markazlashgan, oddiy forma, orqa fonda xira animatsiyalangan
     chorraha (Home sahifasidagi kabi, lekin ko'proq xiralashtirilgan)
   - Forma o'zi glassmorphism kartochka ichida

── ANIMATSIYALAR — QANDAY ISHLASHI KERAK ──────────────────────────

A) ASOSIY ANIMATSIYA (Canvas — ScenarioPlayer):
   - Bu qism DIZAYNER TOMONIDAN CHIZILMAYDI (dasturiy hisoblanadi),
     lekin dizayner UNING ATROFINI (frame, fon, boshqaruv tugmalari,
     progress scrubber) loyihalaydi
   - Talab: Canvas atrofida yumshoq radius (16-20px), yengil soya,
     "HUD" hissi beruvchi ingichka chegara chizig'i (border-glow)
   - Playback tugmalari (▶/⏸, tezlik 0.25x-1x): pastki panelda,
     minimalistik, glassmorphism fonda

B) MIKRO-ANIMATSIYALAR (butun sayt bo'ylab):
   - Tugma hover: 150-200ms transition, biroz katta bo'lish (scale 1.02)
     + glow soya paydo bo'lishi
   - Sahifa almashtirish: fade + slight slide (200-300ms), keskin
     "sakrash" bo'lmasligi kerak
   - Kartochkalar: sahifa yuklanganda pastdan yuqoriga fade-in,
     ketma-ket 50-100ms farq bilan chiqishi (stagger effekt)
   - Natija bannerlari (✅/💥/⚠️): pastdan slide-in + biroz "bounce"
     (spring animatsiya), 400-500ms

C) MUHIM PRINSIP — ANIMATSIYA TA'LIM JARAYONIGA XALAQIT BERMASLIGI:
   Har qanday dekorativ animatsiya (parallax, fade, stagger) FAQAT
   fon yoki UI elementlarida bo'lishi kerak. Asosiy Canvas
   animatsiyasi (mashina harakati) hech qachon boshqa vizual effekt
   bilan aralashtirilmasligi kerak — bu foydalanuvchi diqqatini
   buzadi va ta'lim samaradorligini pasaytiradi.

── RESPONSIVE TALABLAR ────────────────────────────────────────────

Breakpointlar: 1024px va 900px (mavjud loyihada shu qiymatlar bor)
Mobilda: Canvas animatsiya maydoni ekran kengligiga moslashadi,
sidebar (Lessons sahifasida) pastga tushib, tab shaklida ko'rinadi

── YETKAZIB BERISH KUTILADIGAN NARSALAR (deliverables) ────────────

☐ Figma fayl: barcha 6+ sahifa uchun desktop va mobil versiya
☐ Har bir interaktiv element uchun holat (default/hover/active/disabled)
☐ Animatsiya spesifikatsiyasi: har bir micro-animatsiya uchun
  davomiylik (ms) va easing turi (masalan ease-out, spring)
☐ Ranglar, shrift o'lchamlari, spacing qadamlari — dizayn tokenlar
  jadvali (mavjud CSS o'zgaruvchilariga mos)
☐ Ikonka to'plami (chorraha, svetofor, imtiyoz belgisi va h.k.)

════════════════════════════════════════════════════════════════
YAKUNIY MAQSAD: Foydalanuvchi ilovani ochganda "bu oddiy o'quv
ilovasi emas, bu deyarli o'yin kabi jonli simulyator" degan
taassurotga ega bo'lishi kerak — lekin professional va ishonchli
ko'rinishni yo'qotmasdan (bu haydovchilik imtihoniga tayyorgarlik,
o'yin emas).
════════════════════════════════════════════════════════════════
```

Xohlasangiz, shu brifga mos ravishda **Practice sahifasi uchun Figma-ga tayyor komponent tavsifi** (aniq piksel o'lchamlar, spacing bilan) yozib beraman — kerakmi?
